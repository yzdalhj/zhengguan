...
import type { Video, SEOMetadata } from '@/types';
...
const seo = computed((): SEOMetadata => {
  if (!video.value) {
    return {
      title: '视频不存在 - 影视分镜参考库',
      description: '请求的视频不存在',
    };
  }
  return {
    title: `${video.value.title} - 帧观`,
    description: video.value.description || `${video.value.title} - 优质影视分镜参考片段`,
    keywords: video.value.tags?.map(t => t.name).join(', '),
    openGraph: {
      title: `${video.value.title} - 帧观`,
      description: video.value.description || `${video.value.title} - 优质影视分镜参考片段`,
      image: video.value.thumbnail_url,
      type: 'video',
      video: video.value.embed_url,
    },
  };
});
...
const toggleCollection = async () => {
  ...
  try {
    if (isCollected.value) {
      await api.delete(`/api/user/collections/${id}`);
      isCollected.value = false;
    } else {
      await api.post(`/api/user/collections/${id}`);
